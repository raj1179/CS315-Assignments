...
function render() {
   ...
   var lpos = vec4(radius * Math.sin(phi), eye_Y, radius * Math.cos(phi), 0.0);
   gl.uniform4fv(light.position, mult(mv, lpos));
   // gl.uniform4fv(light.position, lpos);

   var matStack = [];
   //Set initial view
   eye = vec3(radius * Math.sin(phi), eye_Y, radius * Math.cos(phi));
   ...
   gl.uniform4fv(material.diffuse, yellow);
